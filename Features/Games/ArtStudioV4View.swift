...
struct ArtStudioV4View: View {
    ...
    @State private var mode: ArtV4Mode?
    @State private var showLegacy = false
    private var isGreek: Bool { appSettings.resolvedLanguage == .greek }

    var body: some View {
        ZStack {
            LinearGradient(colors: [.purple.opacity(0.86), .blue.opacity(0.80)], startPoint: .topLeading, endPoint: .bottomTrailing)
                .ignoresSafeArea()
            ScrollView {
                VStack(spacing: 18) {
        ...
        .fullScreenCover(item: $mode) { ArtV4Canvas(mode: $0).environmentObject(appSettings) }
        .fullScreenCover(isPresented: $showLegacy) { NavigationStack { ArtStudioFixedView().environmentObject(appSettings) } }
    }

    private func card(_ mode: ArtV4Mode, _ emoji: String, _ title: String) -> some View {
...
private enum ArtV4Mode: String, Identifiable { case free, trace, color, challenge; var id: String { rawValue } }
private enum ArtV4Tool: String, CaseIterable, Identifiable { case pencil, pen, marker, brush, eraser; var id: String { rawValue } }
private struct ArtV4Page: Identifiable { let id: Int; let gr: String; let en: String }
private struct ArtV4Color: Identifiable { let id: String; let swift: Color }

private struct ArtV4Canvas: View {
    @EnvironmentObject private var appSettings: AppSettings
    @Environment(\.dismiss) private var dismiss
    let mode: ArtV4Mode

    @State private var canvas = PKCanvasView()
    @State private var tool: ArtV4Tool = .pencil
    ...
    private let colors: [ArtV4Color] = [
        .init(id: "black", swift: .black), .init(id: "white", swift: .white), .init(id: "gray", swift: .gray),
        .init(id: "red", swift: .red), .init(id: "orange", swift: .orange), .init(id: "yellow", swift: .yellow),
        .init(id: "green", swift: .green), .init(id: "mint", swift: .mint), .init(id: "cyan", swift: .cyan),
        .init(id: "blue", swift: .blue), .init(id: "purple", swift: .purple), .init(id: "pink", swift: .pink),
        .init(id: "brown", swift: .brown)
    ]
    ...
    private var challengeHeader: some View {
        ...
        return HStack {
            Text(isGreek ? gr[challenge] : en[challenge]).font(.headline.weight(.black)).multilineTextAlignment(.center)
            Button { challenge = (challenge + 1) % gr.count } label: { Image(systemName: "dice.fill") }
        }
        .padding(12)
        .frame(maxWidth: .infinity)
    ...
    private func updateTool() {
        ...
        canvas.tool = PKInkingTool(ink, color: ArtStudioColorMapping.uiColor(for: colorID), width: width)
    }

    private func icon(_ tool: ArtV4Tool) -> String {
                ...
                LazyVGrid(columns: [GridItem(.adaptive(minimum: 145), spacing: 12)], spacing: 12) {
                    ForEach(pages) { page in
                        Button {
                            ...
                            VStack(spacing: 7) {
                                ArtStudioExactPageView(index: page.id).frame(height: 105).padding(5).background(.white).clipped()
...
private struct ArtV4PKCanvas: UIViewRepresentable {
