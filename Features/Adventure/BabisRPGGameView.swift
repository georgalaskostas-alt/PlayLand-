...
enum RPGEncounter: Identifiable, Equatable {
    case animal(kind: String)
    case treasure(style: String)
    case unicorn
    case fox

    var id: String {
        switch self {
        case .animal(let kind): return "animal-\(kind)"
        case .treasure(let style): return "treasure-\(style)"
...
final class RPGGameState: ObservableObject {
    ...
    @Published var pendingEncounter: RPGEncounter?
    ...
    var areaNumberText: String { "\(area.rawValue + 1)/\(RPGArea.allCases.count)" }
    ...
    var objectiveItems: [RPGObjective] {
        ...
        switch area {
        case .forest: return [o("apple", "🍎", 8), o("wood", "🪵", 5), o("water", "💧", 4), o("rescue", "🐾", 1), o("puzzle", "🧠", 1)]
        case .rescueClearing: return [o("berries", "🫐", 8), o("carrot", "🥕", 6), o("rescue", "🐾", 2), o("puzzle", "🧠", 1), o("treasure", "🧰", 1)]
        case .village: return [o("wood", "🪵", 8), o("berries", "🫐", 6), o("carrot", "🥕", 6), o("rescue", "🐾", 1), o("treasure", "🧰", 1)]
        case .riverCrossing: return [o("water", "💧", 8), o("wood", "🪵", 6), o("key", "🗝️", 2), o("rescue", "🐾", 1), o("puzzle", "🧩", 1)]
        case .puzzleClearing: return [o("crystal", "💎", 6), o("key", "🗝️", 3), o("puzzle", "🧩", 3), o("treasure", "🧰", 2)]
        case .crystalCave: return [o("crystal", "💎", 12), o("key", "🗝️", 4), o("puzzle", "🧩", 2), o("rescue", "🐾", 1), o("treasure", "🧰", 1)]
        case .nightForest: return [o("fragment", "🗺️", 8), o("feather", "⭐", 5), o("rescue", "🐾", 2), o("puzzle", "🧩", 1)]
        case .unicornGrove: return [o("crystal", "💎", 8), o("fragment", "🗺️", 4), o("puzzle", "🧩", 2), o("unicorn", "🦄", 1), o("treasure", "🧰", 1)]
        case .treasureClearing: return [o("key", "🗝️", 5), o("crystal", "💎", 8), o("treasure", "🧰", 3), o("puzzle", "🧩", 2)]
        case .foxDen: return [o("fox", "🦊", 1), o("puzzle", "🧩", 1), o("treasure", "🧰", 1)]
        }
    }
    ...
    var areaGoalComplete: Bool { objectiveItems.allSatisfy { $0.current >= $0.target } }
    ...
    func rescueAnimal(kind: String) {
        ...
        setMessage(greek: "Το \(animalGreekName(kind)) είναι ασφαλές! Συνέχισε την εξερεύνηση.", english: "The \(kind) is safe! Keep exploring.")
        announceGoalIfReady()
    }

    func requestChallenge(_ challenge: RPGChallenge) { pendingChallenge = challenge }

    func completeChallenge(_ challenge: RPGChallenge) {
        pendingChallenge = nil
        areaCounters["puzzle", default: 0] += 1
        AudioManager.shared.play(.starReward)
        setMessage(greek: "Μπράβο! Έλυσες τον γρίφο. Η περιπέτεια συνεχίζεται!", english: "Great job! You solved the puzzle. The adventure continues!")
        announceGoalIfReady()
    }
    ...
    func openTreasure() {
        ...
        AudioManager.shared.play(.starReward)
        setMessage(greek: "Θησαυρός! Μπράβο — άνοιξες το κρυμμένο σεντούκι.", english: "Treasure! Great job — you opened the hidden chest.")
        announceGoalIfReady()
    }
    ...
    func helpUnicorn() {
        ...
        AudioManager.shared.play(.starReward)
        setMessage(greek: "Η μαγεία επέστρεψε! Ο μονόκερος σε ευχαριστεί.", english: "The magic is back! The unicorn thanks you.")
        announceGoalIfReady()
    }

    func meetFox() {
        areaCounters["fox"] = 1
        setMessage(greek: "Η Αλεπού δεν θέλει να πολεμήσει. Άκουσε την ιστορία της και βρες τον τελευταίο θησαυρό.", english: "The Fox does not want to fight. Hear her story and find the final treasure.")
    ...
    func advanceArea() {
        ...
        if area == .foxDen {
            ...
            setMessage(greek: "Μεγάλη Περιπέτεια ολοκληρώθηκε! Ο Μπάμπης, το Κοτσύφι και οι φίλοι τους έσωσαν όλο το δάσος!", english: "Great Adventure complete! Babis, Kotsifi and their friends saved the whole forest!")
            return
        }
    ...
    func setMessageForCurrentArea() {
        switch area {
        case .forest: setMessage(greek: "Κεφάλαιο 1: εξερεύνησε το Μεγάλο Δάσος. Μάζεψε προμήθειες, σώσε το κουνελάκι και άνοιξε το πρώτο μαγικό σεντούκι.", english: "Chapter 1: explore the Great Forest. Gather supplies, rescue the rabbit and open the first magic chest.")
        case .rescueClearing: setMessage(greek: "Κεφάλαιο 2: στο Ξέφωτο Διάσωσης δύο ζωάκια χρειάζονται βοήθεια. Ψάξε κάθε γωνιά.", english: "Chapter 2: two animals need help in Rescue Clearing. Search every corner.")
        case .village: setMessage(greek: "Κεφάλαιο 3: το Χωριό χρειάζεται προμήθειες. Μίλα στους φίλους, βρες το κρυμμένο σεντούκι και ετοίμασε το πέρασμα.", english: "Chapter 3: the Village needs supplies. Help your friends, find the hidden chest and prepare the crossing.")
        case .riverCrossing: setMessage(greek: "Κεφάλαιο 4: πέρασε το ποτάμι. Βρες ξύλα, νερό και τα δύο κλειδιά εξερευνώντας ελεύθερα την περιοχή.", english: "Chapter 4: cross the river. Find wood, water and two keys while exploring the area.")
        case .puzzleClearing: setMessage(greek: "Κεφάλαιο 5: τρεις διαφορετικοί γρίφοι προστατεύουν το Ξέφωτο. Λύσε τους και βρες δύο θησαυρούς.", english: "Chapter 5: three different puzzles guard the Clearing. Solve them and find two treasures.")
        case .crystalCave: setMessage(greek: "Κεφάλαιο 6: μπες βαθιά στην Κρυστάλλινη Σπηλιά. Βρες τους κρυστάλλους, τα κλειδιά και το χαμένο ζωάκι.", english: "Chapter 6: travel deep into Crystal Cave. Find the crystals, keys and the lost animal.")
        case .nightForest: setMessage(greek: "Κεφάλαιο 7: νύχτωσε. Βρες τα κομμάτια του χάρτη και τα Χρυσά Φτερά και βοήθησε δύο φίλους.", english: "Chapter 7: night has fallen. Find map pieces and Golden Feathers and help two friends.")
        case .unicornGrove: setMessage(greek: "Κεφάλαιο 8: ο μονόκερος έχασε τη μαγεία του. Βρες τους κρυστάλλους και λύσε τους γρίφους για να τον βοηθήσεις.", english: "Chapter 8: the unicorn lost its magic. Find the crystals and solve the puzzles to help it.")
        case .treasureClearing: setMessage(greek: "Κεφάλαιο 9: η μεγάλη αναζήτηση θησαυρού. Τρία σεντούκια, δύο γρίφοι και πέντε κλειδιά κρύβονται στο ξέφωτο.", english: "Chapter 9: the great treasure hunt. Three chests, two puzzles and five keys are hidden in the clearing.")
        case .foxDen: setMessage(greek: "Τελικό Κεφάλαιο: βρες την Αλεπού, λύσε τον τελευταίο γρίφο και άνοιξε τον τελευταίο θησαυρό.", english: "Final Chapter: find the Fox, solve the last puzzle and open the final treasure.")
        }
    }
    ...
    private func announceGoalIfReady() {
        if areaGoalComplete {
            setMessage(greek: area == .foxDen ? "Όλα είναι έτοιμα. Πήγαινε στην έξοδο για να ολοκληρώσεις τη Μεγάλη Περιπέτεια!" : "Όλοι οι στόχοι ολοκληρώθηκαν! Βρες την έξοδο για να περάσεις στην επόμενη περιοχή.", english: area == .foxDen ? "Everything is ready. Go to the exit to finish the Great Adventure!" : "Every objective is complete! Find the exit to reach the next area.")
        }
    }
...
struct BabisRPGGameView: View {
        ...
        GeometryReader { geometry in
            ZStack {
                ...
                if let encounter = gameState.pendingEncounter {
                    centeredBackdrop
                    RPGEncounterTaskView(
                        encounter: encounter,
                        isGreek: gameState.isGreek,
                        onSolved: { scene?.confirmPendingEncounter() },
                        onCancel: { scene?.cancelPendingEncounter() }
                    )
                    .frame(maxWidth: 680, maxHeight: 470)
                    .padding(28)
                    .zIndex(101)
                }
            }
    ...
    private var centeredBackdrop: some View {
        ...
        VStack(spacing: 0) {
            ...
            HStack(alignment: .bottom) {
                ...
                if gameState.nearbyAction != nil {
                    interactionButton.transition(.scale.combined(with: .opacity))
                }
            }
    ...
    private var playerCard: some View {
        HStack(spacing: 9) {
            Image(uiImage: UIImage(named: "babis_rpg_idle") ?? UIImage(named: "babis_rpg_master") ?? UIImage())
                .resizable().scaledToFit().frame(width: 58, height: 58).padding(3)
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 5) {
                    ...
                    Text(gameState.areaNumberText)
                        .font(.caption2.weight(.black)).padding(.horizontal, 5).padding(.vertical, 2)
                        .background(.white.opacity(0.15)).clipShape(Capsule())
                }
                .font(.system(size: 17, weight: .black, design: .rounded)).foregroundStyle(.white)
                Text(gameState.currentAreaTitle).font(.caption.weight(.bold)).foregroundStyle(.white.opacity(0.82)).lineLimit(1)
        ...
        .background(.black.opacity(0.55)).clipShape(RoundedRectangle(cornerRadius: 18))
    }

    private var objectivePanel: some View {
        VStack(alignment: .leading, spacing: 5) {
            Text(gameState.isGreek ? "ΑΠΟΣΤΟΛΗ" : "QUEST").font(.caption2.weight(.black)).foregroundStyle(.white.opacity(0.78))
            HStack(spacing: 4) {
        ...
        .padding(8).background(.black.opacity(0.48)).clipShape(RoundedRectangle(cornerRadius: 16))
    }

    private var miniMap: some View {
        ZStack {
            Image(gameState.area.groundAsset).resizable().scaledToFill().frame(width: 72, height: 72).clipShape(Circle())
            Circle().stroke(PlayLandColors.sunOrange, lineWidth: 3).frame(width: 72, height: 72)
            Image(systemName: "location.north.fill").font(.title3.weight(.black)).foregroundStyle(.white).shadow(color: .black, radius: 3)
        }
    }
    ...
    private var companionButton: some View {
        Button { scene?.talkToCompanion() } label: {
            Image(uiImage: UIImage(named: "kotsifi_rpg_idle") ?? UIImage(named: "kotsifi_rpg_master") ?? UIImage())
                .resizable().scaledToFit().frame(width: 46, height: 46).padding(6)
    ...
    private var inventoryButton: some View {
        Button { withAnimation { showInventory.toggle() } } label: {
            Image(systemName: "backpack.fill").font(.title3.weight(.black)).foregroundStyle(.white)
                .frame(width: 50, height: 50).background(.black.opacity(0.52)).clipShape(Circle())
        }.buttonStyle(.plain)
    }
    ...
    private var interactionButton: some View {
        Button { scene?.performInteraction() } label: {
            VStack(spacing: 5) {
                Image(systemName: gameState.interactionIcon).font(.system(size: 25, weight: .black))
                Text(gameState.interactionTitle).font(.caption.weight(.black)).lineLimit(2).multilineTextAlignment(.center)
            }
            .foregroundStyle(.white).frame(width: 98, height: 98)
    ...
    private var inventoryPanel: some View {
        ZStack(alignment: .trailing) {
            ...
            VStack(alignment: .leading, spacing: 14) {
                HStack {
                    Label(gameState.isGreek ? "Σακίδιο" : "Inventory", systemImage: "backpack.fill").font(.title2.weight(.black))
                    Spacer()
                    Button { withAnimation { showInventory = false } } label: { Image(systemName: "xmark.circle.fill").font(.title2) }
                }
                LazyVGrid(columns: [GridItem(.flexible()), GridItem(.flexible()), GridItem(.flexible())], spacing: 10) {
                    ForEach(Array(gameState.inventoryItems.enumerated()), id: \.offset) { _, item in
                        VStack(spacing: 4) {
                            AppAssets.image(item.0).resizable().scaledToFit().frame(height: 42)
                            Text("×\(item.2)").font(.headline.monospacedDigit())
                            Text(item.1).font(.caption2).multilineTextAlignment(.center).lineLimit(2)
                        }
                        .frame(maxWidth: .infinity, minHeight: 90).padding(7)
                        .background(PlayLandColors.warmCream.opacity(0.96)).clipShape(RoundedRectangle(cornerRadius: 14))
                    }
                }
            }
            .padding(18).frame(width: 380).frame(maxHeight: .infinity)
            .background(.ultraThickMaterial).clipShape(RoundedRectangle(cornerRadius: 24))
    ...
    private func objectiveChip(icon: String, value: String, complete: Bool) -> some View {
        HStack(spacing: 2) {
            Text(icon)
            Text(value).font(.caption2.weight(.black).monospacedDigit()).foregroundStyle(.white)
            if complete { Image(systemName: "checkmark.circle.fill").font(.caption2).foregroundStyle(PlayLandColors.leafGreen) }
        }
        .padding(.horizontal, 6).padding(.vertical, 5).background(.white.opacity(0.12)).clipShape(Capsule())
    }
}
...
private struct RPGPuzzleChallengeView: View {
    ...
    private var prompt: String {
        switch challenge {
        case .memory: return isGreek ? "Θυμήσου: 🍎 ⭐ 🍎. Τι ήταν στη μέση;" : "Remember: 🍎 ⭐ 🍎. What was in the middle?"
    ...
    private var choices: [(String, Bool)] {
        switch challenge {
        case .memory: return [("⭐", true), ("🍎", false), ("💧", false)]
    ...
    private var lockAsset: String {
        ...
        case .shapes: return "rpg_lock_shapes"
        ...
        VStack(spacing: 14) {
            HStack {
                ...
                Button(action: onCancel) {
                    Image(systemName: "xmark.circle.fill").font(.system(size: 34, weight: .bold)).foregroundStyle(.white)
                }
            }
            Image(lockAsset).resizable().scaledToFit().frame(maxHeight: 150)
            Text(prompt).font(.title3.weight(.bold)).foregroundStyle(.white).multilineTextAlignment(.center)
            HStack(spacing: 14) {
                ForEach(Array(choices.enumerated()), id: \.offset) { _, choice in
                    Button {
                        if choice.1 {
                            feedback = isGreek ? "Μπράβο!" : "Great!"
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.30) { onSolved() }
                        } else {
                            feedback = isGreek ? "Δοκίμασε ξανά!" : "Try again!"
                            AudioManager.shared.play(.wrong)
                        }
                    } label: {
                        Text(choice.0).font(.system(size: 22, weight: .black, design: .rounded)).foregroundStyle(.white)
                            .frame(minWidth: 115, minHeight: 62).background(PlayLandColors.sunOrange).clipShape(RoundedRectangle(cornerRadius: 16))
                    }
                }
            }
            Text(feedback).font(.headline.weight(.black)).foregroundStyle(.white).frame(height: 24)
        }
        .padding(24)
        .background(LinearGradient(colors: [.indigo.opacity(0.96), .black.opacity(0.96)], startPoint: .topLeading, endPoint: .bottomTrailing))
        .clipShape(RoundedRectangle(cornerRadius: 28))
        .overlay(RoundedRectangle(cornerRadius: 28).stroke(.white.opacity(0.35), lineWidth: 2))
        .shadow(color: .black.opacity(0.55), radius: 24, y: 10)
    }
}

private struct RPGEncounterTaskView: View {
    let encounter: RPGEncounter
    let isGreek: Bool
    let onSolved: () -> Void
    let onCancel: () -> Void
    @State private var feedback = ""
    ...
    private var asset: String {
        switch encounter {
        case .animal(let kind): return "\(kind)_rpg_worried"
        case .treasure(let style): return style == "crystal" ? "rpg_chest_crystal_closed" : (style == "magic" ? "rpg_chest_magic_closed" : "rpg_chest_wood_closed")
        case .unicorn: return "unicorn_rpg_worried"
        case .fox: return "fox_friendly"
        }
    }

    private var title: String {
        switch encounter {
        case .animal: return isGreek ? "Βοήθησε το ζωάκι" : "Help the animal"
        case .treasure: return isGreek ? "Άνοιξε το σεντούκι" : "Open the chest"
        case .unicorn: return isGreek ? "Βοήθησε τον μονόκερο" : "Help the unicorn"
        case .fox: return isGreek ? "Μίλα στην Αλεπού" : "Talk to the Fox"
        }
    }

    private var prompt: String {
        switch encounter {
        case .animal: return isGreek ? "Τι θα βοηθούσε περισσότερο ένα κουρασμένο ζωάκι;" : "What would help a tired animal most?"
        case .treasure: return isGreek ? "Τι χρειάζεσαι για να ανοίξεις το κλειδωμένο σεντούκι;" : "What do you need to open the locked chest?"
        case .unicorn: return isGreek ? "Τι μπορεί να επαναφέρει τη μαγεία του μονόκερου;" : "What can restore the unicorn's magic?"
        case .fox: return isGreek ? "Η Αλεπού θέλει να σου πει κάτι. Τι πρέπει να κάνεις;" : "The Fox wants to tell you something. What should you do?"
        }
    }

    private var choices: [(String, Bool)] {
        switch encounter {
        case .animal: return [(isGreek ? "ΝΕΡΟ 💧" : "WATER 💧", true), (isGreek ? "ΠΕΤΡΑ 🪨" : "ROCK 🪨", false), (isGreek ? "ΚΛΕΙΔΙ 🗝️" : "KEY 🗝️", false)]
        case .treasure: return [(isGreek ? "ΚΛΕΙΔΙ 🗝️" : "KEY 🗝️", true), (isGreek ? "ΜΗΛΟ 🍎" : "APPLE 🍎", false), (isGreek ? "ΦΤΕΡΟ ⭐" : "FEATHER ⭐", false)]
        case .unicorn: return [(isGreek ? "ΚΡΥΣΤΑΛΛΟΣ 💎" : "CRYSTAL 💎", true), (isGreek ? "ΞΥΛΟ 🪵" : "WOOD 🪵", false), (isGreek ? "ΚΑΡΟΤΟ 🥕" : "CARROT 🥕", false)]
        case .fox: return [(isGreek ? "ΝΑ ΑΚΟΥΣΩ 👂" : "LISTEN 👂", true), (isGreek ? "ΝΑ ΦΥΓΩ" : "LEAVE", false), (isGreek ? "ΝΑ ΚΡΥΦΤΩ" : "HIDE", false)]
        }
    }
    ...
    var body: some View {
        VStack(spacing: 14) {
            HStack {
                Text(title).font(.system(size: 27, weight: .black, design: .rounded)).foregroundStyle(.white)
                Spacer()
                Button(action: onCancel) {
                    Image(systemName: "xmark.circle.fill").font(.system(size: 34, weight: .bold)).foregroundStyle(.white)
                }
            }
            Image(asset).resizable().scaledToFit().frame(maxHeight: 150)
            Text(prompt).font(.title3.weight(.bold)).foregroundStyle(.white).multilineTextAlignment(.center)
            HStack(spacing: 12) {
                ForEach(Array(choices.enumerated()), id: \.offset) { _, choice in
                    Button {
                        if choice.1 {
                            feedback = isGreek ? "Μπράβο!" : "Great!"
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.28) { onSolved() }
                        } else {
                            feedback = isGreek ? "Δοκίμασε ξανά!" : "Try again!"
                            AudioManager.shared.play(.wrong)
                        }
                    } label: {
                        Text(choice.0).font(.system(size: 18, weight: .black, design: .rounded)).foregroundStyle(.white)
                            .frame(minWidth: 125, minHeight: 60).background(PlayLandColors.sunOrange).clipShape(RoundedRectangle(cornerRadius: 15))
                    }
                }
            }
            Text(feedback).font(.headline.weight(.black)).foregroundStyle(.white).frame(height: 24)
        ...
        .padding(24)
        .background(LinearGradient(colors: [Color.green.opacity(0.93), Color.black.opacity(0.95)], startPoint: .topLeading, endPoint: .bottomTrailing))
        .clipShape(RoundedRectangle(cornerRadius: 28))
        .overlay(RoundedRectangle(cornerRadius: 28).stroke(.white.opacity(0.38), lineWidth: 2))
        .shadow(color: .black.opacity(0.55), radius: 24, y: 10)
    }
}
...
private struct RPGJoystick: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle().fill(.white.opacity(0.23)).frame(width: knobDiameter, height: knobDiameter)
                .overlay(Circle().stroke(.white.opacity(0.35), lineWidth: 1)).offset(knobOffset)
        }
