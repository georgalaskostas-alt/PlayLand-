...
final class BabisRPGScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        g.cancelsTouchesInView = false; view.addGestureRecognizer(g); pinch = g
        AudioManager.shared.playLoop(named: "rpg_adventure_theme", volume: 0.24)
        loadArea(gameState.area)
    }
    override func willMove(from view: SKView) {
        if let pinch { view.removeGestureRecognizer(pinch) }
        AudioManager.shared.stopMusic(); SpeechManager.shared.stop()
    }

    func setMovementVector(_ vector: CGVector) {
        guard canMove else { return }
        let m = min(1, hypot(vector.dx, vector.dy)); joystickMagnitude = m
        if m < 0.04 { joystick = .zero }
        else { joystick = CGVector(dx: vector.dx / m, dy: vector.dy / m); tapTarget = nil }
    }
    func stopMovement() { joystick = .zero; joystickMagnitude = 0; tapTarget = nil; setMovement(.idle) }

    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        guard canMove, joystickMagnitude < 0.04, let touch = touches.first else { return }
        let p = touch.location(in: world)
        let target = CGPoint(x: min(max(p.x, 110), worldSize.width - 110), y: min(max(p.y, 110), worldSize.height - 110))
        tapTarget = nearestWalkable(to: target)
    }
    ...
    func talkToCompanion() {
        let messages = [
            ("Εξερεύνησε ελεύθερα. Μπορείς να πας παντού εκτός από πάνω στα φυσικά εμπόδια.", "Explore freely. You can go anywhere except through natural obstacles."),
            ("Ψάξε όλη την περιοχή. Οι φίλοι και τα αντικείμενα βρίσκονται πάντα σε προσβάσιμα σημεία.", "Search the whole area. Friends and items are always placed in reachable spots."),
            ("Στην άκρη κάθε περιοχής υπάρχει το πέρασμα για το επόμενο μεγάλο μέρος της περιπέτειας.", "At the edge of each area is the passage to the next part of the adventure.")
        ]
        let m = messages[gameState.area.rawValue % messages.count]
        gameState.setMessage(greek: m.0, english: m.1); SpeechManager.shared.speak(text: gameState.message)
    }

    func performInteraction() {
        guard let node = nearby, let name = node.name else { return }
        stopMovement()
        if name.hasPrefix("puzzle:"), let raw = name.split(separator: ":").dropFirst().first, let challenge = RPGChallenge(rawValue: String(raw)) {
            activeChallenge = node; gameState.requestChallenge(challenge); return
        }
        if name.hasPrefix("treasure:") {
            activeEncounter = node; gameState.pendingEncounter = .treasure(style: String(name.split(separator: ":")[1])); return
        }
        if name.hasPrefix("npc:") {
            let p = name.split(separator: ":"); guard p.count >= 3 else { return }
            let kind = String(p[1])
            if p[2] == "worried" { activeEncounter = node; gameState.pendingEncounter = .animal(kind: kind) }
            else { talkAnimal(node, kind: kind) }
            return
        }
        ...
        if name == "area_exit" {
            guard gameState.areaGoalComplete else {
                gameState.setMessage(greek: "Ολοκλήρωσε πρώτα τους στόχους της περιοχής.", english: "Complete this area's objectives first."); AudioManager.shared.play(.wrong); return
            }
            transitionThrough(node)
        }
    }

    func confirmPendingEncounter() {
        guard let node = activeEncounter, let e = gameState.pendingEncounter else { return }
        gameState.pendingEncounter = nil
        switch e {
        case .animal(let kind):
            gameState.rescueAnimal(kind: kind); node.name = "npc:\(kind):happy"; node.userData?["action"] = "animalTalk"; pulse(node); heart(node)
        case .treasure: openTreasure(node)
        case .unicorn:
            gameState.helpUnicorn(); node.texture = tex("unicorn_rpg_happy", "unicorn_rpg_neutral"); node.name = "unicorn:happy"; pulse(node); heart(node)
        case .fox: gameState.meetFox(); node.texture = tex("fox_talking", "fox_friendly"); pulse(node)
        }
        activeEncounter = nil; SpeechManager.shared.speak(text: gameState.message)
    }
    func cancelPendingEncounter() { gameState.pendingEncounter = nil; activeEncounter = nil }
    func completeActiveChallenge() {
        guard let node = activeChallenge else { return }
        node.texture = tex("rpg_chest_magic_open", "rpg_chest_magic_closed"); node.name = "solved"
        interactions.removeAll { $0 === node }; activeChallenge = nil; nearby = nil; gameState.nearbyAction = nil
        reward(at: node.position)
    }

    @objc private func handlePinch(_ g: UIPinchGestureRecognizer) {
        guard !transitioning else { return }
        if g.state == .began || g.state == .changed {
            cameraNode.setScale(min(maxCameraScale, max(minCameraScale, cameraNode.xScale / g.scale))); g.scale = 1; clampCamera()
        }
    }

    private func loadArea(_ area: RPGArea) {
        transitioning = true; stopMovement(); world.removeAllChildren(); interactions.removeAll(); collectibles.removeAll(); obstacles.removeAll(); reserved.removeAll()
        nearby = nil; activeChallenge = nil; activeEncounter = nil; gameState.nearbyAction = nil; gameState.pendingChallenge = nil; gameState.pendingEncounter = nil
        let bg = SKSpriteNode(imageNamed: background(for: area)); bg.size = worldSize; bg.position = CGPoint(x: worldSize.width/2, y: worldSize.height/2); bg.zPosition = -5000; world.addChild(bg)
        buildEnvironment(area); buildGameplay(area); addAmbientCreatures(area); addExit(area)
        let spawn = safePoint(near: CGPoint(x: 900, y: 900), footprint: CGSize(width: 150, height: 130), reserve: false)
        setupPlayer(spawn); setupCompanion(); cameraNode.setScale(1.05); cameraNode.position = spawn; clampCamera(); gameState.setMessageForCurrentArea(); transitioning = false
    }

    private func background(for area: RPGArea) -> String {
        switch area {
        ...
        case .riverCrossing: return "rpg_rescue_clearing"
        ...
        case .unicornGrove: return "rpg_forest_ground_01"
        case .treasureClearing: return "rpg_treasure_clearing"
        case .foxDen: return "rpg_fox_area_ground"
        }
    }

    private func buildEnvironment(_ area: RPGArea) {
        // Keep the playable center open; scenery forms real collision islands instead of fake path walls.
        let trees = [(650,1450),(1250,2500),(700,3900),(1900,4750),(3050,4950),(4300,4700),(5650,5000),(7000,4650),(7900,3800),(7700,2300),(8050,1050),(5900,900),(4300,1200),(2700,1050)]
        for (i,v) in trees.enumerated() { scenery(i % 2 == 0 ? "rpg_tree_large_01" : "rpg_tree_large_02", CGPoint(x:v.0,y:v.1), CGSize(width:390,height:520), 0.72, 0.62) }
        let bushes = [(1600,1600),(2300,2100),(3400,1800),(5000,1750),(6400,1550),(7150,2900),(6100,3600),(4700,3850),(3300,3600),(2000,3450)]
        for (i,v) in bushes.enumerated() { scenery(i % 2 == 0 ? "rpg_bush_tall_01" : "rpg_bush_tall_02", CGPoint(x:v.0,y:v.1), CGSize(width:270,height:230), 0.96, 0.88) }
        let rocks = [(1150,850),(2850,2850),(4050,2450),(5350,2850),(6750,2350),(7450,900)]
        for (i,v) in rocks.enumerated() { scenery(i % 2 == 0 ? "rpg_rock_large" : "rpg_rock_small", CGPoint(x:v.0,y:v.1), CGSize(width:230,height:190), 0.92, 0.86) }
        scenery("rpg_fallen_tree", CGPoint(x:1900,y:2850), CGSize(width:470,height:230), 0.96, 0.76)
        scenery("rpg_tree_stump", CGPoint(x:5550,y:2300), CGSize(width:220,height:195), 0.92, 0.78)
        if area == .village {
            scenery("rpg_village_house_01", CGPoint(x:1850,y:4300), CGSize(width:720,height:640), 0.90, 0.72)
            scenery("rpg_village_house_02", CGPoint(x:6700,y:4200), CGSize(width:720,height:640), 0.90, 0.72)
        }
        if area == .riverCrossing {
            obstacles += [CGRect(x:0,y:2600,width:3500,height:620), CGRect(x:5100,y:2600,width:3500,height:620)]
            let bridge=SKSpriteNode(imageNamed:"rpg_bridge_wood");bridge.position=CGPoint(x:4300,y:2910);bridge.size=CGSize(width:1700,height:620);bridge.zPosition = -1000;world.addChild(bridge)
        }
        if area == .crystalCave { scenery("rpg_cave_entrance", CGPoint(x:7350,y:4450), CGSize(width:720,height:580), 0.88, 0.70) }
    }

    private func scenery(_ asset:String,_ p:CGPoint,_ s:CGSize,_ wf:CGFloat,_ hf:CGFloat) {
        let n=SKSpriteNode(imageNamed:asset);n.position=p;n.size=s;n.zPosition=depth(p.y);world.addChild(n)
        obstacles.append(CGRect(x:p.x-s.width*wf/2,y:p.y-s.height*hf/2,width:s.width*wf,height:s.height*hf))
    }

    private func buildGameplay(_ a:RPGArea) {
        switch a {
    ...
    private let anchors:[CGPoint] = stride(from: 0, to: 30, by: 1).map { i in
        let col=i%6,row=i/6; return CGPoint(x:1500+CGFloat(col)*1150,y:1150+CGFloat(row)*900)
    }
    private func anchor(_ i:Int)->CGPoint { anchors[(i + gameState.area.rawValue*3) % anchors.count] }

    private func pickups(_ kind:String,_ asset:String,_ count:Int) {
        for i in 0..<count {
            let base=anchor((i*2+kind.count)%anchors.count); let p=safePoint(near:CGPoint(x:base.x+CGFloat((i%3)-1)*180,y:base.y+CGFloat((i%4)-2)*130),footprint:CGSize(width:110,height:100))
            let n=SKSpriteNode(imageNamed:asset);n.position=p;n.size=CGSize(width:72,height:72);n.name="collect:\(kind)";n.zPosition=depth(p.y)+4;world.addChild(n);collectibles.append(n)
            n.run(.repeatForever(.sequence([.moveBy(x:0,y:8,duration:0.7),.moveBy(x:0,y:-8,duration:0.7)])))
        }
    }
    ...
    private func addAmbientCreatures(_ area:RPGArea) {
        for (i,id) in RPGCreatureCatalog.population(for: area).enumerated() {
            guard id != "rabbit", let c=RPGCreatureCatalog.creature(id) else { continue }
            let size:CGFloat = c.collisionScale > 0.78 ? 205 : (c.collisionScale < 0.55 ? 105 : 155)
            npc(c.asset,"ambient:\(id)",12+i,size,nil)
        }
    }

    private func addExit(_ area:RPGArea) {
        let asset:String
        switch area { case .forest,.nightForest:asset="rpg_wood_sign";case .rescueClearing:asset="rpg_village_house_01";case .village:asset="rpg_bridge_wood";case .riverCrossing:asset="rpg_puzzle_clearing";case .puzzleClearing:asset="rpg_crystal_cave_entrance";case .crystalCave:asset="rpg_cave_entrance";case .unicornGrove:asset="rpg_treasure_clearing";case .treasureClearing:asset="rpg_fox_den";case .foxDen:asset="rpg_chest_crystal_closed" }
        npc(asset,"area_exit",29,area == .rescueClearing ? 330:210,.exit)
    }

    private func npc(_ asset:String,_ name:String,_ index:Int,_ size:CGFloat,_ action:RPGNearbyAction?) {
        let p=safePoint(near:anchor(index),footprint:CGSize(width:max(130,size*0.85),height:max(110,size*0.70)))
        let n=SKSpriteNode(imageNamed:asset);n.position=p;n.size=CGSize(width:size,height:size);n.name=name;n.zPosition=depth(p.y)+6;n.userData=NSMutableDictionary();if let action{n.userData?["action"]=actionKey(action)};world.addChild(n)
        if action != nil { interactions.append(n) }
        let w=size*(name.hasPrefix("ambient:") ? 0.62:0.78),h=size*0.62;obstacles.append(CGRect(x:p.x-w/2,y:p.y-h*0.42,width:w,height:h));pulse(n)
    }

    private func safePoint(near desired:CGPoint, footprint:CGSize, reserve shouldReserve:Bool=true)->CGPoint {
        let offsets:[CGPoint]=[.zero,.init(x:220,y:0),.init(x:-220,y:0),.init(x:0,y:220),.init(x:0,y:-220),.init(x:330,y:220),.init(x:-330,y:220),.init(x:330,y:-220),.init(x:-330,y:-220),.init(x:520,y:0),.init(x:-520,y:0)]
        for o in offsets { let p=CGPoint(x:min(max(desired.x+o.x,220),worldSize.width-220),y:min(max(desired.y+o.y,220),worldSize.height-220));let r=CGRect(x:p.x-footprint.width/2,y:p.y-footprint.height/2,width:footprint.width,height:footprint.height);if !obstacles.contains(where:{$0.intersects(r)}) && !reserved.contains(where:{$0.intersects(r.insetBy(dx:-70,dy:-70))}) { if shouldReserve{reserved.append(r)};return p } }
        for row in 0..<8 { for col in 0..<12 { let p=CGPoint(x:700+CGFloat(col)*650,y:650+CGFloat(row)*600);let r=CGRect(x:p.x-footprint.width/2,y:p.y-footprint.height/2,width:footprint.width,height:footprint.height);if !obstacles.contains(where:{$0.intersects(r)}) && !reserved.contains(where:{$0.intersects(r.insetBy(dx:-60,dy:-60))}) {if shouldReserve{reserved.append(r)};return p} } }
    ...
    private func talkAnimal(_ n:SKSpriteNode,kind:String){gameState.setMessage(greek:"Ο φίλος σου σε ευχαριστεί! Συνέχισε την εξερεύνηση.",english:"Your friend thanks you! Keep exploring.");SpeechManager.shared.speak(text:gameState.message);pulse(n)}
