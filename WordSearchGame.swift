    ...
    var body: some View {
        ZStack {
            VStack(spacing: 18) {
                ...
                Text("Tap the letters in order to spell a word!")
                    .font(.headline)
                ...
                HStack(spacing: 12) {
                    ForEach(targetWords, id: \.self) { word in
                        Text(word)
                            ...
                            .padding(.vertical, 6)
                            .background(foundWords.contains(word) ? PlayLandTheme.leafGreen.opacity(0.3) : PlayLandTheme.skyBlue.opacity(0.12))
                ...
                HStack(spacing: 16) {
                    ...
                    Button("Check Word") { checkSelection() }
                        .buttonStyle(PlayfulButtonStyle(color: PlayLandTheme.skyBlue))
    ...
    private func letterCell(row: Int, col: Int) -> some View {
        ...
        return Button(action: { tap(pos) }) {
            Text(grid.isEmpty ? "" : String(grid[row][col]))
                ...
                .background(
                    isFound ? PlayLandTheme.leafGreen.opacity(0.35)
                    : (isSelected ? (showWrong ? Color.red.opacity(0.4) : PlayLandTheme.sunOrange.opacity(0.4)) : PlayLandTheme.skyBlue.opacity(0.1))
                )
